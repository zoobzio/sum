...
```go
// Wrong
sum.NewDatabase[User](db, "users", renderer) // panics

// Correct
svc := sum.New(sum.ServiceConfig{...})
sum.NewDatabase[User](db, "users", renderer)
```
...
**Cause:** Invalid database connection or table name.

**Solution:** Verify database connectivity before creation:

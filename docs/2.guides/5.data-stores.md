...
`NewDatabase` returns an error from store creation:

```go
database, err := sum.NewDatabase[Model](db, table, renderer)
if err != nil {
    // grub.NewDatabase failed—check connection or table
    log.Fatalf("failed to create database: %v", err)
}
```

`NewStore` and `NewBucket` do not return errors—they create the wrapper synchronously.
